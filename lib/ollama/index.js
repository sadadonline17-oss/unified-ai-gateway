...
const CLOUD_MODELS = {
  ...
  code_generate: 'qwen3-coder-next',  // Qwen3-Coder-Next - FREE, agentic coding

  // === ADVANCED CODE MODELS ===
  advanced_code: 'glm-5',  // GLM-5 744B(40B active) - FREE, agentic coding, reasoning

  // === ALL AVAILABLE FREE CLOUD MODELS ===
  // Chat Models:
  all_chat_models: [
    'qwen3.5',              // 397B-A17B MoE - Best overall chat, vision
    'kimi-k2.5',            // Multimodal, vision-language, agentic
    'minimax-m2.5',         // Productivity, coding
    'glm-5',                // 744B(40B active) - Reasoning, agentic
    'qwen3-next',           // 80B - Tools, thinking
    'nemotron-3-nano',      // 30B - Tools, thinking, agentic
    'ministral:14b',        // 14B - Vision, tools, edge
    'ministral:8b',         // 8B - Edge deployment
    'rnj-1',                // 8B - Tools, code, STEM
    'llama3.2:7b',          // 7B - Fast, balanced
    'llama3.2:3b',          // 3B - Ultra fast
    'gemma2:9b',            // 9B - Google Gemma 2
    'mistral:7b',           // 7B - Mistral v0.3
    'phi3:14b',             // 14B - Microsoft Phi-3
  ],

  // Code Models:
  all_code_models: [
    'qwen3-coder-next',     // Best agentic coding
    'deepseek-v3',          // 671B MoE - Excellent coding
    'deepseek-v3.2',        // Improved reasoning, agent performance
    'deepseek-r1:8b',       // 8B - Reasoning, coding
    'glm-5',                // 744B - Agentic coding
    'glm-4.7',              // Coding specialized
    'glm-4.6',              // Agentic, reasoning, coding
    'qwen2.5-coder:32b',    // 32B - Qwen 2.5 Coder
    'qwen2.5-coder:7b',     // 7B - Fast coding
    'codellama:7b',         // 7B - Meta CodeLlama
    'devstral-small-2',     // 24B - Vision, tools, code
    'devstral-2',           // 123B - SE agents
    'minimax-m2',           // Coding, agentic
    'minimax-m2.1',         // Multilingual, code engineering
    'cogito-2.1',           // 671B - Instruction-tuned, MIT license
  ],

  // Vision Models:
  all_vision_models: [
    'qwen3-vl:32b',         // 32B - Best vision
    'qwen3-vl:8b',          // 8B - Balanced vision
    'qwen3-vl:4b',          // 4B - Fast vision
    'llama3.2-vision:11b',  // 11B - Llama 3.2 Vision
    'llama3.2-vision:90b',  // 90B - Llama 3.2 Vision Large
  ],
};
...
export class OllamaProvider extends EventEmitter {
  ...
  async listModels() {
    if (this.useCloudModels) {
      return CLOUD_MODELS.all_chat_models.map(name => ({ name, cloud: true }));
    }
    const response = await this._request('GET', '/api/tags');
  ...
  getCloudModels() {
    return {
      chat: CLOUD_MODELS.all_chat_models,
      code: CLOUD_MODELS.all_code_models,
      vision: CLOUD_MODELS.all_vision_models,
      current: {
        chat: this.modelRoutes.core_chat,
        code: this.modelRoutes.code_generate,
        advanced: this.modelRoutes.advanced_code
      }
    };
  }
   ...
   * Set model by name
   */
  setModel(modelName, mode = 'chat') {
    if (mode === 'chat' || mode === 'core_chat') {
      this.modelRoutes.core_chat = modelName;
    } else if (mode === 'code' || mode === 'code_generate') {
      this.modelRoutes.code_generate = modelName;
    } else if (mode === 'advanced' || mode === 'advanced_code') {
      this.modelRoutes.advanced_code = modelName;
    }
    this.emit('model:changed', { mode, model: modelName });
  }
  ...
  async getModelInfo(modelName) {
    if (this.useCloudModels) {
      return { name: modelName, cloud: true, details: 'Free cloud model via Ollama' };
    }
  ...
  async deleteModel(modelName) {
    if (this.useCloudModels) {
      throw new Error('Cannot delete cloud models');
    }
